...
def test_registration(setup):
    ...
    name_field.send_keys(generate_email())
    email_field.send_keys(generate_email())
    password_field.send_keys('q')
    register_btn.click()
    # Необходимо дождаться смены цвета
    time.sleep(0.5)
    assert register_btn.text == 'Зарегистрироваться'
    assert driver.find_element(By.CSS_SELECTOR, '.input__error.text_type_main-default').text == 'Некорректный пароль'
    assert (driver.find_element(By.CSS_SELECTOR, '.input_status_error').value_of_css_property('border-color') ==
            'rgb(229, 43, 26)')
